...
#define OW_PKMN_OBJECTS_SHARE_PALETTES TRUE      // BUBBA: turned on. [WIP!! NOT ALL PALETTES HAVE BEEN ADJUSTED FOR THIS!!] If TRUE, follower palettes are taken from battle sprites.
...
// Follower Pokémon
#define OW_FOLLOWERS_ENABLED           TRUE      // BUBBA: turned on.Enables follower Pokémon, HGSS style. Requires OW_POKEMON_OBJECT_EVENTS. Note that additional scripting may be required for them to be fully supported!
...
#define OW_FOLLOWERS_COPY_WILD_PKMN    TRUE      //BUBBA: turned on. If TRUE, follower Pokémon that know Transform or have Illusion/Imposter will copy wild Pokémon at random.
#define OW_BATTLE_ONLY_FORMS           TRUE       //BUBBA: turned on. If TRUE, loads overworld sprites for battle-only forms like Mega Evos. Requires OW_POKEMON_OBJECT_EVENTS.
