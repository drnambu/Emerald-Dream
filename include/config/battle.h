...
#define B_WAIT_TIME_MULTIPLIER              8    // BUBBA: updated to 8. This determines how long text pauses in battle last. Vanilla is 16. Lower values result in faster battles.
...
#define B_AFFECTION_MECHANICS           FALSE       //BUBBA: Turned off. In Gen6+, there's a stat called affection that can trigger different effects in battle. From LGPE onwards, those effects use friendship instead.
...
#define B_USE_FROSTBITE                 TRUE      // BUBBA: turned on frostbite. In PLA, Frostbite replaces Freeze. Enabling this flag does the same here. Moves can still be cherry-picked to either Freeze or Frostbite. Freeze-Dry, Secret Power & Tri Attack depend on this config.
...
#define B_PARTNER_MONS_MARKED_SEEN      TRUE      // BUBBA: turned on. If TRUE, if your double battle partner sends out a Pokémon you haven't encountered yet, it will be marked as SEEN in your Pokédex.
...
#define B_SHOW_TYPES        SHOW_TYPES_CAUGHT     // BUBBA: turned on if caught pokemon. When to show type indicators next to Pokémon health bars in battle, while choosing a move after selecting a target Pokémon.
...
#define B_SHOW_EFFECTIVENESS        SHOW_EFFECTIVENESS_CAUGHT //BUBBA: turned on if caught pokemon. If not SHOW_EFFECTIVENESS_NEVER, the PP string is replaced by a type effectiveness indicator based off the moves and the opposing side.
